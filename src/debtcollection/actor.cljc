(ns debtcollection.actor
  "DebtCollectionActor — the ISCO-08 4214 community debtcollection actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one debtcollection operation request
  ...
  The unconditional invariant: the DebtCollectionAdvisor can never
  directly commit a record the DebtCollectionGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [debtcollection.advisor :as advisor]
            [debtcollection.governor :as governor]
            [debtcollection.store :as store]))

(defn build-graph
  "Build a compiled DebtCollectionActor graph. `store` implements
  `debtcollection.store/Store`. `advisor` implements
  `debtcollection.advisor/Advisor` (defaults to `mock-advisor`).
